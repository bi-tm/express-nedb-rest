...
function addRestMethods(router) {
    ...
    router.get('/:collection', function (req, res, next) {
        var where = filter(req.query.filter);
        if (typeof(req.query.count) == "undefined") {
          req.nedb.find(where, function(err, docs) {
              if (err) {
                  return next(err);
              }
              else {
          ...
          req.nedb.count(where, function(err, count) {
                      if (err) {
                          return next(err);
                      }
                      else {
    ...
    router.delete('/:collection', function (req, res, next) {
        var where = filter(req.param.filter);
        req.nedb.remove(where, { multi: true }, function (err, count) {
            if (err) {
                return next(err);
            }
            if (count != 1) {
    ...
    router.put('/:collection/:id', function (req, res, next) {
        ...
        var where = filter(req.param.filter);
        req.nedb.update(where, req.body, {multi:true}, function (err, count, doc) {
            if (err) {
                return next(err);
            }
            if (count != 1) {
    ...
    router.put('/:collection', function (req, res, next) {
        ...
        var where = filter(req.param.filter);
        req.nedb.update(where, req.body, {multi:false}, function (err, count, doc) {
            if (err) {
                return next(err);
            }
            if (count != 1) {
